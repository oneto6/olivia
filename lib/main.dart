...
Widget createDialog(String title) {
  return AlertDialog(
    title: Text(title),
    actions: [
      ElevatedButton(
...
Future<void> handleMsg(RemoteMessage? msg) async {
  ...
  var dialog = createDialog(msg.notification?.title ?? "no title");
  if (ctx == null || ctx.mounted == false) {
...
void main() async {
  ...
  var fDeviceID = await FirebaseInstallations.instance.getId();
  debugPrint('firebase_app_installations id: $fDeviceID');
